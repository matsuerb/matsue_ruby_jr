...
ball.on(:hit, wall) do
  ...
  if (score % 5) == 0
    # 課題C-1: ラケットを小さくする
    if racket.image.height > 40
      racket.image = Image.new(racket.image.width, racket.image.height - 10)
      racket.fill(color: "yellowgreen")
    end

    # 課題C-4: ラケットが小さくなりすぎたらスピードを元に戻して100点のボーナスを加える
    if racket.image.height <= 40
      racket.image = Image.new(racket.image.width, 120)
      racket.fill(color: "yellowgreen")
      score += 100
      speed = 10
    end
